...
public class SimpleConcurrentSolutionV2 {
    ...
    private static class ResourceCheckTask implements Runnable {
        ResourceCheckTaskWorker checkInstance = new ResourceCheckTaskWorker(); 
        @Override
        public void run() {
            for (int i = 1; i <= 5; i++) {
                Thread tCheck = new Thread(checkInstance, ""+i);
                tCheck.start();
            }
        }
    }
    ...
    private static class ResourceCheckTaskWorker implements Runnable {
        ...
        public void run() {
            ...
            String processNumber = currentThread.getName();

            try {
                // Gera um valor aleatório entre 1000ms (1s) e 3000ms (3s)
                int sleepTime = 1000 + random.nextInt(2000); 
                System.out.println("[" + "Check-Recursos " + currentThread.getName() + "] Verificando Recurso " + processNumber + " (Duração: " + sleepTime + "ms)...");
                Thread.sleep(sleepTime); 
            } catch (InterruptedException e) {
                System.err.println("[" + "Check-Recursos " + currentThread.getName() + "] Verificação interrompida.");
                Thread.currentThread().interrupt();
                return;
            }

            System.out.println("[" + "Check-Recursos " + currentThread.getName() + "] FIM: Verificação concluída.");
        }
    }
